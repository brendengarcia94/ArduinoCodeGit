...
void adjust_pmt_sensitivity_voltage(double voltage, double pmt_vref_value)
{
  ...
  int number_of_adc_samples_to_average = 2;
  ...
  double tuning_resistance = 10000*voltage/pmt_vref_value;
  ...
  double read_buffer[number_of_adc_samples_to_average];
  
  prev_voltage = get_averaged_voltage_from_adc(PMT_SENSITIVITY_VOLTAGE_PIN, 0, 5);
  //wait_for_data_and_continue(); //DEBUG
  //prev_voltage = average_array(read_buffer, number_of_adc_samples_to_average);
  if(prev_voltage > voltage)
  {
    /* ERROR */
    Serial.println(F("[ERROR] Voltage Not acheivable"));
    Serial.print(F("[ERROR] Prev Voltage: "));
    Serial.println(prev_voltage);
    Serial.print(F("[ERROR] Voltage: "));
  ...
  {
    transfer_value_to_pot(PMT_ADJUST_CHANNEL, tuning_word);
    adc_voltage = get_averaged_voltage_from_adc(PMT_SENSITIVITY_VOLTAGE_PIN, 0, 5);
    if ((adc_voltage > voltage) && (prev_voltage <= voltage))
    {
      double dist_below = abs(voltage - prev_voltage);
...
double get_voltage_from_adc(int analog_pin, double vrefl, double vrefh, int number_of_samples)
{
  double voltage_samples[number_of_samples];
  int adc_value;
  for(int i = 0; i < number_of_samples; i++)
  {
    adc_value = analogRead(analog_pin);
    voltage_samples[i] = vrefl + (double)adc_value * ((vrefh-vrefl)/1024.0);
    voltage_samples[i] = voltage_samples[i] / number_of_samples;
  }
  double voltage = 0;
  for (int i = 0; i < number_of_samples; i++)
  {
    voltage = voltage + voltage_samples[i];
  }
  return voltage;
}

double get_averaged_voltage_from_adc(int analog_pin, double vrefl, double vrefh)
{
  int number_of_samples = 13; //6 Samples seems to optimize changes from averages of samples
                            // 19 samples seems to optimize rate of rate of change (which should be close to zero)
                            //uses 13 to get in the middle of these two
  return get_voltage_from_adc(analog_pin, vrefl, vrefh, number_of_samples);
}

double average_array(double* array, int array_size)
{
  double average;
  for(int i = 0; i < array_size; i++)
  {
    average = array[i]/((double)array_size);
  }
  return average;
}
