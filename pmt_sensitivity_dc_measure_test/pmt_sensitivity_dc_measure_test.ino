...
void loop() {
  ...
  int averages[max_average];
  for (int i = 0; i < max_average; i++)
  {
    averages[i] = i+1;
  }
  
  double voltages[max_average];
  for (int i = 0; i < 256; i++)
  {
    for (int j = 0; j < max_average; j++)
    {
      transfer_value_to_pot(1, 0);
      delayMicroseconds(100);
      transfer_value_to_pot(1, i);
      delayMicroseconds(100);
      voltages[j] = get_voltage_from_adc(PMT_SENSITIVITY_VOLTAGE_PIN, 0, 5, averages[j]); 
    }
    Serial.print(i);
