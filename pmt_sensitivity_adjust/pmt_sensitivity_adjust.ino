...
void loop() {
  //analogReference(EXTERNAL); //test
  Serial.println(F("[DEBUG] Enter number between .4-1.1 V"));
  ...
  char **_;
  double value_read = strtod(serial_input, _);
  Serial.print(F("[DEBUG] Value Read: "));
  Serial.println(value_read, 4);
  adjust_pmt_sensitivity_voltage(value_read, PMT_VREF_VOLTAGE);
  Serial.print(F("[DEBUG] Voltage Read Back in: "));
  Serial.println(get_voltage_from_adc(A2, 0, 5), 5);
  Serial.flush();
