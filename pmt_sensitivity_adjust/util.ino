...
void adjust_pmt_sensitivity_voltage(double voltage, double pmt_vref_value)
{
  ...
  prev_voltage = get_voltage_from_adc(PMT_SENSITIVITY_VOLTAGE_PIN, 0, 5);
  ...
  //adc_value = read_in_adc(PMT_SENSITIVITY_VOLTAGE_PIN, 5);
  //prev_voltage = adc_to_voltage(adc_value);
  prev_voltage = get_voltage_from_adc(PMT_SENSITIVITY_VOLTAGE_PIN, 0, 1.4);
  ...
  while(adjusted == 0)
  {
    ...
    adc_voltage = get_voltage_from_adc(PMT_SENSITIVITY_VOLTAGE_PIN, 0, 5);
